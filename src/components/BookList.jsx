...
const BookList = () => {
  ...
  const [pagination, setPagination] = useState({}); // Nuevo estado para la paginación

  // Función para obtener los libros, ahora con un parámetro de página
  const fetchBooks = (pageUrl) => {
    setLoading(true);
    axios
      .get(pageUrl)
      .then((response) => {
        ...
        if (Array.isArray(fetchedBooks)) {
          ...
          setPagination(response.data); // Guarda la info de paginación
        } else {
          setBooks([]);
          setPagination({});
        }
  ...
  useEffect(() => {
    fetchBooks("http://localhost:8000/api/books");
  }, []);
  ...
  return (
    <div className="container mx-auto p-6">
      ...
      {/* Buscador */}
      <div className="col-3 justify-center mb-6">
        <input
          type="text"
      ...
      <div className="grid grid-cols-3 gap-x-4 gap-y-4 ">
        ...
        {!loading && filteredBooks.length > 0 && (
            <div className="flex button-0 relative justify-center items-center space-x-4 mt-8">
                <button
                    onClick={() => fetchBooks(pagination.prev_page_url)}
                ...
                <button
                    onClick={() => fetchBooks(pagination.next_page_url)}
      ...
      {isModalOpen && selectedBook && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-6 z-50">
            <div className="relative bg-gradient-to-br from-indigo-900 via-blue-900 to-indigo-800 text-white rounded-2xl shadow-2xl w-full max-w-4xl flex overflow-hidden animate-fadeIn">
            ...
            <div className="w-2/3 p-8 flex flex-col justify-between">
                ...
                {/* Botones de acción */}
                <div className="relative right-1 mt-6 flex gap-4">
                <Link 

    ...
    useEffect(() => {
        if (id) {
            ...
            axios.get(`http://localhost:8000/api/books/${id}`)
                .then(response => {
                    // Actualiza el estado con los datos existentes del libro
                    // Y asegúrate de manejar los nuevos campos que pueden ser nulos
                    setBook({
    ...
    return (
        <div className="max-w-md mx-auto bg-gradient-to-br from-indigo-600 to-purple-700 p-8 rounded-xl shadow-2xl transform hover:scale-105 transition-transform duration-300 ease-in-out border border-indigo-700">
            <div className="flex items-center justify-center space-x-3 text-white mb-6">
                {id ? <PencilSquareIcon className="h-10 w-10" /> : <PlusIcon className="h-10 w-10" />}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                    ...
                    <input
                        ...
                        className="w-full px-5 py-3 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-purple-400 focus:border-transparent text-gray-800 bg-white bg-opacity-90 placeholder-gray-500 transition duration-200"
                ...
                <div>
                    <label htmlFor="genre" className="block text-white text-lg font-semibold mb-2">
                        Género
                    </label>
                    <select
                        id="genre"
                        ...
                        value={book.genre}
                        onChange={handleChange}
                        className="w-full px-5 py-3 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-purple-400 focus:border-transparent text-gray-800 bg-white bg-opacity-90 transition duration-200"
                    >
                ...
                <div>
                    <label htmlFor="published_year" className="block text-white text-lg font-semibold mb-2">
                        Año de Publicación
                    </label>
                    <input
                        type="number"
                ...
                <div>
                    <label htmlFor="pages" className="block text-white text-lg font-semibold mb-2">
                        Número de Páginas
                    </label>
                    <input
                        type="number"
                ...
                <div>
                    <label htmlFor="cover_url" className="block text-white text-lg font-semibold mb-2">
                        URL de la Portada
                    </label>
                    <input
                        type="url"
                ...
                <div>
                    <label htmlFor="reading_status" className="block text-white text-lg font-semibold mb-2">
                        Estado de Lectura
                    </label>
                    <select
                        id="reading_status"
                ...
                <button
                    type="submit"
                    className="w-full bg-yellow-400 text-indigo-900 py-3 rounded-lg font-extrabold text-xl shadow-md hover:bg-yellow-300 focus:outline-none focus:ring-4 focus:ring-yellow-500 focus:ring-opacity-75 transition-all duration-300 ease-in-out transform hover:-translate-y-1"
                    disabled={loading}
                >
                    {loading ? 'Guardando...' : (id ? 'Actualizar Libro' : 'Añadir Libro')}
                </button>
            </form>
        </div>
    );
